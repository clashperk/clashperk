...
export default class SetupUtilsCommand extends Command {
  ...
  public async exec(
    ...
    if (args.option === 'reminder-ping-exclusion') {
      return this.reminderPingExclusion(interaction, args);
    }

    const command = {
      'role-refresh-button': this.handler.getCommand('setup-buttons'),
      'link-button': this.handler.getCommand('setup-buttons'),
      'events-schedular': this.handler.getCommand('setup-events'),
      'flag-alert-log': this.handler.getCommand('setup-server-logs'),
      'roster-changelog': this.handler.getCommand('setup-server-logs'),
      'maintenance-break-log': this.handler.getCommand('setup-server-logs')
    }[args.option];
  ...
  private async reminderPingExclusion(
    interaction: CommandInteraction<'cached'>,
