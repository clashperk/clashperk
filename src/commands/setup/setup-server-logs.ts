...
export default class SetupUtilsCommand extends Command {
  ...
  public async flagAlertLog(
    ...
    if (args.disable) {
      await this.client.db
        .collection(Collections.FLAG_ALERT_LOGS)
    ...
    createInteractionCollector({
      ...
      onClick: async (action) => {
        if (action.customId === customIds.useAutoRole) {
          ...
          useAutoRoleButton.setLabel(
            state.useAutoRole ? 'Use the Selected Role' : 'Use Clan-Specific Roles'
          );
  ...
  public async welcomeLog(interaction: CommandInteraction<'cached'>, args: { disable?: boolean }) {
    const state = this.client.settings.get<WelcomeLogConfig>(
      interaction.guildId,
      Settings.WELCOME_LOG,
      {
        enabled: true,
    ...
    if (args.disable) {
      this.client.settings.set(interaction.guildId, Settings.WELCOME_LOG, {
        ...state,
        enabled: false
      });
      return interaction.editReply('Successfully disabled.');
    ...
    const onCustomization = async (action: ButtonInteraction<'cached'>) => {
      ...
      const welcomeTextInput = new TextInputBuilder()
        ...
        .setPlaceholder(
          'This will be shown in the welcome message. \nWelcome {{user}} \nMust include {{user}}'
        )
