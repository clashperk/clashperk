...
export default class RedeemCommand extends Command {
  ...
  public async exec(interaction: CommandInteraction<'cached'>, { disable }: { disable?: boolean }) {
    ...
    if (!result) {
      return interaction.editReply({
        content:
          '**Something went wrong (unresponsive api), please [contact us.](<https://discord.gg/ppuppun>)**'
      });
    }
    ...
    result.data.sort((a, b) => {
      return (
        new Date(b.attributes.last_charge_date).getTime() -
        new Date(a.attributes.last_charge_date).getTime()
      );
    });
    ...
    const disabledUserIds = this.client.settings.get<string[]>(
      'global',
    ...
    if (disable) {
      if (!user || !user.guilds.length)
        return interaction.editReply('**You do not have an active subscription.**');
      return this.disableRedemption(interaction, {
        select: true,
        user,
        message: { content: '**Manage Patreon Subscriptions**' }
      });
    }
    ...
    if (!pledge) {
      return interaction.editReply(
        '**Something went wrong (unknown pledge), please [contact us.](<https://discord.gg/ppuppun>)**'
      );
    }
    ...
    if (pledge.attributes.patron_status !== 'active_patron' && !isGifted) {
      return interaction.editReply(
        '**Something went wrong (declined pledge), please [contact us.](<https://discord.gg/ppuppun>)**'
      );
    }
  ...
  private async disableRedemption(
    interaction: CommandInteraction,
    {
      ...
      user,
      select
    }: { message: WebhookMessageEditOptions; user: WithId<PatreonMembersEntity>; select: boolean }
  ) {
    const collection = this.client.db.collection<PatreonMembersEntity>(Collections.PATREON_MEMBERS);
    const customIds = {
    ...
    collector.on('collect', async (action) => {
      ...
      if (action.customId === customIds.menu && action.isStringSelectMenu()) {
        ...
        if (!guild) {
          await action.update({
            content:
              '**Something went wrong (unknown server), please [contact us.](<https://discord.gg/ppuppun>)**'
          });
          return;
        }
        ...
        await action.editReply({
          components: [],
          content: `Subscription disabled for **${guild.name} (${guild.id})**`
        });
      }
    });
  }
