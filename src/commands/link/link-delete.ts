...
export default class LinkDeleteCommand extends Command {
  ...
  public async exec(
    ...
    args: { clan_tag?: string; player_tag?: string }
  ) {
    ...
    if (clanTag) {
      ...
      if (unlinked) {
        return interaction.editReply(
          this.i18n('command.link.delete.success', {
      ...
      return interaction.editReply(
        this.i18n('common.no_match_found', { lng: interaction.locale, tag: `**${clanTag}**` })
      );
    }
    ...
    if (
      !(interaction.user.id === member.id || linkedByUserIds.includes(member.linkedBy)) &&
      ...
      if (!players.length) {
        return interaction.editReply(
          this.i18n('command.link.delete.no_access', {
            lng: interaction.locale,
    ...
    return interaction.editReply(
      this.i18n('command.link.delete.success', { lng: interaction.locale, tag: `**${playerTag}**` })
    );
  }
