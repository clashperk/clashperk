  ...
  public async run(message: Message<true>, { id }: { id: string }) {
    ...
    if (blacklist.includes(user.id)) {
      ...
      return message.channel.send(
        `**${user.displayName}** has been removed from the ${this.client.user.displayName}'s blacklist.`
      );
    }
    ...
    return message.channel.send(
      `**${user.displayName}** has been blacklisted from using ${this.client.user.displayName}'s command.`
    );
  }
}
