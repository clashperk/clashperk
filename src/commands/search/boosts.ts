...
export default class BoostsCommand extends Command {
  ...
  public async exec(
    ...
    const players = members.filter(
      (mem) => mem.troops.filter((en) => en.superTroopIsActive).length
    );
    if (!players.length)
      return interaction.followUp({
    ...
    const selected = players
      .filter(
        (mem) => mem.troops.filter((en) => en.name === args.unit && en.superTroopIsActive).length
      )
      .filter((m) => (recently.length && args.recent ? recently.includes(m.tag) : true)).length;

    const boosters = players.filter((m) =>
