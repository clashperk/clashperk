...
class GuideCommand extends Command {
	...
	async exec(message) {
		...
		const embed = this.client.util.embed()
			.setColor(0x5970c1)
			.setAuthor('Guide', this.client.user.displayAvatarURL())
			.setDescription([
				...
				'',
				'**Toggle Logs and Boards**',
				'• _**Clan Log**_',
				`\u2002\`${prefix}toggle clanlog <clanTag>\``,
				'• _**Donationlog Log**_',
				`\u2002\`${prefix}toggle donationlog <clanTag>\``,
				'• _**Last Online Board**_',
				`\u2002\`${prefix}toggle lastonline <clanTag>\``,
				'• _**Clan Games Board**_',
				`\u2002\`${prefix}toggle cgboard <clanTag>\``,
				'',
				'**Linked Clans**',
				`\`${prefix}clans\` This command shows all linked clans related to your discord server / guild.`,
				'',
				'**Remove Clan**',
				`\`${prefix}remove <clanTag>\` This command deletes all logs, boards and all data for the specified clan. So be careful!`,
				'',
				'**Link Comamnd**',
				'• _**Link Clan**_',
