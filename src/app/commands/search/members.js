...
class MembersCommand extends Command {
	...
	*args() {
	...
	async exec(message, { data, townhall }) {
		...
		const header = stripIndent(`${emoji.trophy}**\`\u200eTH ${'TAG'.padStart(10, ' ')} ${'NAME'.padEnd(20, '\u2002')}\`**`);
		const pages = [
			this.paginate(townhall ? filter : items, 0, 25)
				.items.map(member => `${leagueEmoji[member.league]}\`\u200e${this.padStart(member.townHallLevel)} ${member.tag.padStart(10, '\u2002')} ${Util.escapeInlineCode(member.name).padEnd(20, '\u2002')}\``),
