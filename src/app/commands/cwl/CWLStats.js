...
class CWLStatsComamnd extends Command {
	...
	*args() {
	...
	async rounds(message, body, data) {
		...
		return message.util.send({ embed: { description } });
	}
	...
	isWinner(clan, opponent) {
		if (clan.stars > opponent.stars) return emoji.ok;
		if (clan.destructionPercentage > opponent.destructionPercentage) return emoji.wrong;
		return emoji.empty;
	}
}
