		...
		for (const member of clan.memberList) {
			...
			if (this.donateList[key] && member.tag in this.donateList[key]) {
				...
				if (donations && donations > 0) {
					...
					item.donated += `${leagueEmoji[member.league.id]}_**${this.formatNum(donations)}**_ \u2002${Util.escapeItalic(member.name)}\n`;
				}
				...
				if (receives && receives > 0) {
					...
					item.received += `${leagueEmoji[member.league.id]}_**${this.formatNum(receives)}**_ \u2002${Util.escapeItalic(member.name)}\n`;
				}
