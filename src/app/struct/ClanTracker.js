		...
		for (const member of clan.memberList) {
			...
			if (this.donateList[key] && member.tag in this.donateList[key]) {
				...
				if (donations && donations > 0) {
					...
					item.donated += `${leagueEmoji[member.league.id]} _**${donations.toString().padStart(3, '\u2002')}** \u2002${Util.escapeItalic(member.name)}_\n`;
				}
				...
				if (receives && receives > 0) {
					...
					item.received += `${leagueEmoji[member.league.id]} _**${receives.toString().padStart(3, '\u2002')}** \u2002${Util.escapeItalic(member.name)}_\n`;
				}
