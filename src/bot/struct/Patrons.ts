...
export default class Patrons {
	...
	private async _autoDelete(debug = false) {
		...
		for (const patron of patrons) {
			...
			if (pledge && new Date(pledge.attributes.last_charge_date).getTime() >= patron.lastChargeDate.getTime()) {
				await this.collection.updateOne({ _id: patron._id }, {
					$set: {
			...
			if (pledge && !(pledge.attributes.lifetime_support_cents === patron.lifetimeSupport && pledge.attributes.currently_entitled_amount_cents === patron.entitledAmount)) {
				await this.collection.updateOne({ _id: patron._id }, {
					$set: {
			...
			if (patron.active && pledge?.attributes.patron_status === 'former_patron') {
				...
				this.client.logger.info(
					`Declined Patron Deleted ${patron.username} (${patron.userId}/${patron.id})`,
					{ label: 'PATRON' }
				);
			...
			if (!patron.active && (patron.declined || patron.cancelled) && pledge?.attributes.patron_status === 'active_patron') {
				...
				this.client.logger.info(
					`Declined Patron Resumed ${patron.username} (${patron.userId}/${patron.id})`,
					{ label: 'PATRON' }
				);
			}
	...
	public async fetchAPI() {
		const query = qs.stringify({
			...
			'fields[member]': 'last_charge_status,last_charge_date,patron_status,email,pledge_relationship_start,currently_entitled_amount_cents,lifetime_support_cents'
		});
