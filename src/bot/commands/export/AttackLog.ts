			...
			for await (const war of cursor) {
				...
				for (const m of clan.members) {
					for (const atk of m.attacks ?? []) {
						...
						const _previousBestAttack = _isFresh
							? { stars: 0 }
							: this.getPreviousBestAttack(__attacks, atk.defenderTag, atk.attackerTag);
				...
				for (const m of opponent.members) {
					for (const atk of m.attacks ?? []) {
						...
						const _previousBestAttack = _isFresh
							? { stars: 0 }
							: this.getPreviousBestAttack(__defenses, atk.defenderTag, atk.attackerTag);
						const _newStars = Math.max(0, atk.stars - _previousBestAttack.stars);
