...
export default class CWLOverviewCommand extends Command {
	public constructor() {
		super('cwl-overview', {
			category: 'war',
			channel: 'guild',
	...
	private async rounds(interaction: CommandInteraction<'cached'>, body: ClanWarLeagueGroup, clanTag: string) {
		...
		for (const data of wars) {
			...
			if (data.state === 'warEnded') {
				clan.stars += this.client.http.isWinner(data.clan, data.opponent) ? data.clan.stars + 10 : data.clan.stars;
				clan.destruction += data.clan.destructionPercentage * data.teamSize;

				opponent.stars += this.client.http.isWinner(data.opponent, data.clan) ? data.opponent.stars + 10 : data.opponent.stars;
				opponent.destruction += data.opponent.destructionPercentage * data.teamSize;
			}
			...
			if (data.clan.tag === clanTag || data.opponent.tag === clanTag) {
				...
				if (data.state === 'warEnded') {
					stars += this.client.http.isWinner(clan, opponent) ? clan.stars + 10 : clan.stars;
