...
export default class ClanEmbedCommand extends Command {
	...
	public async exec(
		...
		const embed = await clanEmbedMaker(data, { color, description: state.description, requirements: state.accepts });
		...
		if (existing) buttonRow.addComponents(resendButton);
		...
		const onCustomization = async (action: ButtonInteraction<'cached'>) => {
			...
			try {
				...
				embed.setDescription(state.description || null);
				embed.setImage(state.bannerImage || null);

				await modalSubmit.deferUpdate();
				await modalSubmit.editReply({ embeds: [embed], components: [buttonRow] });
				// await interaction.editReply({ embeds: [embed], components: [buttonRow], message: '@original' });
			} catch (error) {
		...
		const onConfirm = async (action: ButtonInteraction<'cached'>) => {
			if (existing) {
				...
				return action.update({ content: '**Clan embed updated!**' });
			}
			...
			return action.update({ content: '**Clan embed created!**' });
		};
