...
export class Migrator {
  ...
  async migrateJoinLeaveLog() {
    this.client.logger.info('Migrating donation logs', { label: 'Migrator' });
    const logs = await this.client.db.collection(Collections.JOIN_LEAVE_LOGS).find(query).toArray();
    ...
    this.client.logger.info('Join leave logs migrated', { label: 'Migrator' });
  }
