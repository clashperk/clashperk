async function update(item: SourceString, maxLength: number) {
  const res = await fetch(
    `https://api.crowdin.com/api/v2/projects/522390/strings/${item.data.id as string}`,
    {
      headers: {
...
(async () => {
  ...
  while (hasMore) {
    const response = await fetch(
      `https://api.crowdin.com/api/v2/projects/522390/strings?fileId=18&limit=${limit}&offset=${offset}`,
      {
        headers: {
          'Authorization': `Bearer ${process.env.CROWDIN_API}`,
          'Content-Type': 'application/json'
  ...
  for (const item of strings) {
    if (
      item.data.identifier.endsWith('.description') &&
      item.data.maxLength !== limits.description
    ) {
